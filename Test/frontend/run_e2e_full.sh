...
if timeout 1 bash -c ':</dev/tcp/127.0.0.1/1420' >/dev/null 2>&1; then
  echo "BLOCKED port 1420 is already in use by an unknown process; E2E will not kill it." >&2
  exit 77
fi
setsid npm run dev >"$OUTPUT_ROOT/vite-full.log" 2>&1 &
vite_pid=$!
printf '%s\n' "$vite_pid" >"$OUTPUT_ROOT/vite-full.pid"
wait_frontend || {
  echo "FAIL Vite frontend did not become reachable on 127.0.0.1:1420" >&2
  exit 1
}
status=0
npm run test:e2e:tauri || status=$?
cleanup
if ! wait_port_closed 8080 120; then
  echo "BLOCKED port 8080 did not release after full E2E; no unknown process was killed." >&2
  status=77
fi
exit "$status"
