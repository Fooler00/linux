...
setsid npm run dev >"$OUTPUT_ROOT/vite-smoke.log" 2>&1 &
vite_pid=$!
printf '%s\n' "$vite_pid" >"$OUTPUT_ROOT/vite-smoke.pid"
wait_frontend || {
  echo "FAIL Vite frontend did not become reachable on 127.0.0.1:1420" >&2
  exit 1
}
status=0
npm run test:e2e:tauri:smoke || status=$?
cleanup
if ! wait_port_closed 1420 60; then
  echo "BLOCKED port 1420 did not release after smoke E2E; no unknown process was killed." >&2
  status=77
fi
if ! wait_port_closed 8080 120; then
  echo "BLOCKED port 8080 did not release after smoke E2E; no unknown process was killed." >&2
  status=77
fi
exit "$status"
